...
class ExtractorPopup {
    ...
    async checkCurrentTab() {
        try {
            ...
            if (tab.url.includes('facebook.com')) {
                this.updateStatus('success', 'На сторінці Facebook', 'Розширення готове до роботи');
                
                if (tab.url.includes('adsmanager')) {
                    this.updateStatus('success', 'На Ads Manager', 'Ідеальна сторінка для витягування!');
                }
            } else {
                this.updateStatus('warning', 'Не на Facebook', 'Перейдіть на facebook.com для використання');
            }
            ...
            this.updateStatus('error', 'Помилка', 'Не вдалося отримати інформацію про сторінку');
        }
    }
    ...
    async extractNow() {
        ...
        try {
            ...
            if (response && response.success) {
                this.updateStatus('success', 'Витягування запущено', 'Очікуйте результати...');
    ...
    async loadSavedResults() {
        try {
            ...
            console.log('Не вдалося завантажити збережені результати:', error.message);
        }
    }
    ...
    displayResults(data) {
        ...
        if (listEl) {
            if (data.accounts.length > 0) {
                ...
                listEl.innerHTML = '<div class="account-item" style="text-align: center; color: #6c757d;">Кабінети не знайдено</div>';
            }
        }
        ...
        if (sourcesEl && data.detailed_data) {
            const sources = [...new Set(data.detailed_data.map(item => item.source))];
        ...
        if (data.total_accounts > 0) {
            this.updateStatus('success', `Знайдено ${data.total_accounts} кабінетів`, 'Готово до відправки!');
        } else {
            this.updateStatus('warning', 'Кабінети не знайдено', 'Спробуйте на сторінці Ads Manager');
        }
    }
    ...
    async sendToBackend() {
        ...
        try {
            const response = await fetch('http://localhost:8000/api/facebook/extension-data', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                this.updateStatus('success', 'Успішно відправлено!', 'Дані збережено в AI-Buyer');
            ...
            this.updateStatus('error', 'Помилка відправки', `Backend недоступний: ${error.message}`);
        } finally {
